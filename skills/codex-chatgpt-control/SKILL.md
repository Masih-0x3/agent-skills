...
1. Do not auto-invoke visible ChatGPT-control workflows.
2. Only run browser-control only when the user request contains an explicit trigger such as:
   - "use ChatGPT now"
   - "invoke codex-chatgpt-control"
   - "run this in ChatGPT"
   - or another direct equivalent instruction to perform the run in this environment.
3. If no explicit trigger is present, do not call `ask`, `askInThread`, `askWithFiles`, `askAndDownload`, `run`, or any other browser-control command.
4. For those cases, return a `Manual ChatGPT handoff` and pause for pasteback.
5. Every response from this skill must include one explicit mode marker:
   - `mode: manual_handoff` when returning a handoff
   - `mode: controlled_run` when you actually run the visible workflow
