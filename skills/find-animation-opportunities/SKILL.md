...
description: Use when the user asks what could be animated or wants an interface to feel more alive; inspect code, reject unnecessary motion, and report precise read-only suggestions without implementing them.
---

# Finding Animation Opportunities

A search skill. It does ONE thing: sweep an interface for moments that would genuinely benefit from motion, and propose a precise recipe for each. It does not review existing animations, audit existing motion, or write implementation code.
...
1. **Never modify source code.** This skill reports; it does not implement. If asked to build a suggestion, provide the recipe as a self-contained implementation handoff.
...
One short paragraph: how much motion this interface actually needs, whether it's already close to right, and which single suggestion has the highest leverage. Close by offering to turn any row into a self-contained implementation plan.
